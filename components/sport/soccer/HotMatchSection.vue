    ...
    <div v-if="hotMatchList.length > 0" :class="isHotMatch ? 'hot-match-bg' : ''">
        <div v-if="isHotMatch" class="title-container">
            <div style="display: flex; margin-left: 1.3rem">
                ...
                <svg style="margin-right: 0.3rem;" xmlns="http://www.w3.org/2000/svg" width="15" height="20"
        ...
        <div v-if="isHotMatch" style="margin: 1rem;" class="hot-match-container">
        ...
        <div v-else style="margin: 1rem;" class="hot-match-container">
            <sport-soccer-event-match-container v-for="(match, index) in hotMatchList" :key="index"
                :tournamentText="match.tournamentText" :matchRunningTime="match.matchRunningTime"
                :isRunningMatch="match.isRunningMatch" :homeName="match.homeName" :homeScore="match.homeScore"
                :homeIcon="match.homeIcon" :homeIconAlt="match.homeIconAlt" :awayName="match.awayName"
                :awayScore="match.awayScore" :awayIcon="match.awayIcon" :awayIconAlt="match.awayIconAlt"
...
async function mapMatch() {
    const matchOddsList = await useSportStore().fetchSportOdds(locale.value, props.marketType, props.leagueId_cigapi, 5)    

    hotMatchList.value = [];
    matchOddsList.forEach((item) => {
        let record = {
            ...
            homeOdds: item.homeOdd_FT_1X2_2,
            awayOdds: item.awayOdd_FT_1X2_2,
            drawOdds: item.odd_FT_1X2_Draw_2
        };
