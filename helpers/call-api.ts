...
export default {
  ...
  async getLeagueStandingContentById(leagueId: String) {
    const { locale } = useI18n();
    ...
    return httpService({
      method: CallApiMethod.get,
      url: "api/league-standings?" + param,
      timeout: 0,
    })
      .then((response) => {
        if (response.data) {
